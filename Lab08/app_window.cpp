...
void AppWindow::initPrograms ()
 {
   ...
   _poly.init ( _prog, GsColor::darkblue, GsColor::darkred );
	...
	GlMaterial material;
	GsColor::brown.get(material.ambient);	GsColor::red.get(material.diffuse);		GsColor::white.get(material.specular);
	_curve.init(_prog2, material, 1.0f);
...
void AppWindow::glutKeyboard ( unsigned char key, int x, int y )
 {
   switch ( key )
    { case ' ': _viewaxis = !_viewaxis; break;
		case '0': _viewcurve = false; break;
		case '1': _curvetype = 1; _viewcurve = true; break;
		case '2': _curvetype = 2; _viewcurve = true; break;
		case '3': _curvetype = 3; _viewcurve = true; break;
		case '4': _curvetype = 4; _viewcurve = true; break;
		case '5': _curvetype = 5; _viewcurve = true; break;
		case 'q': _segments++; break;
		case 'a': _segments--; break;
		case 'z': _build3D = true; _curve.changed = true; break;
		case 'x': _curve.changed = !_curve.changed; break;
...
void AppWindow::glutDisplay ()
 {
   ...
   glClear ( GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT );
	//if(_poly.changed)	
	if(_curve.changed) _curve.build(_poly, _segments, _curvetype, _build3D);

	GlLight _sun;
	{
		_sun.pos = GsVec(0.00f, 0.00f, 1.0f);
...
//	_sun.pos = _sun.pos * _str;
   // Draw:
   if ( _viewaxis ) _axis.draw ( _str, _spr );
   _poly.draw ( _str, _spr );
	if ( _viewcurve) _curve.draw( _str, _spr, _sun);
